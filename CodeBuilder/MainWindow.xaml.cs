...
using CodeBuilderPlus.Properties;
using System.Windows.Controls;
using System.Windows.Forms;
using MySql.Data.MySqlClient;
using System.Threading;

namespace CodeBuilderPlus
{
    /// <summary>
    ...
    public partial class MainWindow : Window
    {
        ...
        private void CreateCode(string tableName)
        {
            ...
            else
            {
                ...
                if (!tbNamespace.Text.Equals("命名空间") && tbNamespace.Text.Trim().Length > 0)
                {
                    ...
                    {
                        realyNamespqce = tbNamespace.Text + ".";
        ...
        private void CreateIDAL(String tbName)
        {
            CreateCodeHelper d = new CreateCodeHelper();
            CreateIDAL(tbName, d);
        ...
        private void Export(string tablename)
        {
            ...
            string fileModel = settings.strPath + "Models" + Path.DirectorySeparatorChar;
        ...
        private string ReplaceNameSpace(string souces, string newName, string oldName = "${namespace}") {

            return souces.Replace(oldName, newName.Trim());
        }
        /// <summary>
        /// 导出helper
        /// </summary>
        private void ExportHelper()
        {
            ...
            CreateDirectory(fileHelper);
            CreateDirectory(helperBase);
            //Helper
            File.WriteAllText(fileHelper + helper + "Helper.cs", ReplaceNameSpace((sqltype == 2 ? settings.MySqlHelper : settings.SqlHelper),realyNamespqce), Encoding.UTF8);
            File.WriteAllText(fileHelper + "GenericSQLGenerator.cs", ReplaceNameSpace( sqltype == 2 ? settings.GenericMySQLGeneratorHelper : settings.GenericSQLGeneratorHelper,realyNamespqce), Encoding.UTF8);
            File.WriteAllText(fileHelper + "CommonExtend.cs", ReplaceNameSpace( settings.CommonExtend, realyNamespqce), Encoding.UTF8);
            //Helper Bases
            File.WriteAllText(helperBase + "BaseSqlHelperConf.cs", ReplaceNameSpace(settings.BaseSqlHelperConf, realyNamespqce), Encoding.UTF8);
            File.WriteAllText(helperBase + "ISqlHelper.cs", ReplaceNameSpace(settings.ISqlHelper, realyNamespqce), Encoding.UTF8);

            //IDAL
            var pathStr = settings.strPath+"IDAL" + Path.DirectorySeparatorChar + "Bases" + Path.DirectorySeparatorChar;
            CreateDirectory(pathStr);
            File.WriteAllText(pathStr + "IBaseDAL.cs", ReplaceNameSpace(settings.IBaseDAL, realyNamespqce), Encoding.UTF8);
            //DAL
        ...
        private void ExportAllCode()
        {
            ...
            for (int i = 0; i < cbTables.Items.Count; i++)
            {
                this.Dispatcher.Invoke(delegate
                {
                    ...
                    CreateIDAL(cbTables.Items[i].ToString());
                });
